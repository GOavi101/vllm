...
def benchmark_memory(
    ...
    requests = [
        request_class(f"req_{i}", num_tokens=100) for i in range(num_requests)
    ]
...
def run_benchmarks(args: argparse.Namespace) -> BenchmarkResults:
    ...
    if not args.skip_memory:
        ...
        print(f"  Creating {format_number(args.num_requests)} requests "
    ...
    if not args.skip_creation:
        ...
        print(f"  Creating and destroying "
        ...
        print(f"  Baseline:  {format_duration(baseline_create['total_time'])} "
              f"({format_number(baseline_create['throughput'])} req/s)")
        print(f"  Optimized: {format_duration(optimized_create['total_time'])} "
    ...
    if not args.skip_access:
        ...
        print(f"  Performing {format_number(args.num_accesses)} "
        ...
        print(f"  Baseline:  {format_duration(baseline_access['total_time'])} "
              f"({format_number(baseline_access['ops_per_sec'])} ops/s)")
        print(f"  Optimized: {format_duration(optimized_access['total_time'])} "
    ...
    if not args.skip_gc:
        ...
        print(f"  Measuring GC pressure with "
              f"{format_number(args.num_iterations)} objects...")

        baseline_gc = benchmark_gc_pressure(
            RequestBaseline, args.num_iterations, True
        )
